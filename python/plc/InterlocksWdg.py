...
2019-10-23 DGatlin  Conerted os.path dependency to pathlib
"""
import os
import time
import tkinter
...
class InterlocksWdg(tkinter.Frame):
    def __init__(self, master, mcpModel):
        ...
        # source since the resulting environment variables
        # will match the ones that already exist

        plc_root_dir = Path(plc.__file__).parent.absolute()

        # The following branching is necessitated by pyinstaller. It will read
        # this module a little weird, and this basically says that if it reads
        # weird, guess that the solution is adding plc/etc instead of etc, which
        # should work in normal circumstances

        count = 0
        while (not (plc_root_dir / 'etc').exists()) and count < 5:
            plc_root_dir = plc_root_dir.parent.absolute()
            count += 1
        if count == 5:
            raise Exception("Couldn't find the root plc directory")
        # print(plc_root_dir)
        os.environ["PLC_DIR"] = str(plc_root_dir)
        plc_etc_dir = plc_root_dir / 'etc/'
        ...
        interlocksTclDir = interlocksRootDir.absolute()
        # print(plc_etc_dir / 'dervish.tcl')
        if (plc_etc_dir / 'dervish.tcl').exists():
            self.tk.call("source", plc_etc_dir / "dervish.tcl")
        else:
        ...
        try:
            self.tk.call("source", plc_etc_dir / "interlockStartup.tcl")
